...
import static guru.qa.niffler.util.DataUtils.createUserJ;
...
public class RestCreateUserExtension extends CreateUserExtension {
    ...
    private UserJson createUserJson(@Nullable String desiredUsername, @Nullable String desiredPassword) {
        Faker faker = new Faker();
        if (desiredUsername == null || desiredPassword == null) {
            return createUserJ(faker.name().username(), faker.internet().password());
        } else {
            return createUserJ(desiredUsername, desiredPassword);
        }
    }
