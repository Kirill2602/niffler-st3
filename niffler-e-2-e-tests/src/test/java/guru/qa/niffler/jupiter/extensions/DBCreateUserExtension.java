...
public class DBCreateUserExtension extends CreateUserExtension {
    ...
    protected UserJson createUserForTest(GenerateUser annotation) throws IOException {
        ...
        if (annotation.username().isEmpty() || annotation.password().isEmpty()){
            user = fillAuthUserEntity(faker.name().username(), faker.internet().password());
        } else {
            user = fillAuthUserEntity(annotation.username(), annotation.password());
        }
    ...
    protected List<UserJson> createFriendsIfPresent(GenerateUser annotation, UserJson currentUser) throws IOException {
        if (annotation.friends().handleAnnotation()) {
            ...
            UserRepository userRepository = new UserRepositoryHibernate();
            UserDataUserDAOHibernate userDataDAOHibernate = new UserDataUserDAOHibernate();
            ...
            for (int i = 0; i < annotation.friends().count(); i++) {
                ...
                userRepository.createUsersFriends(mainUserData, friendUserData);
